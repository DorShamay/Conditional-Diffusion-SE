    ...
    def train(self, max_steps=10000):
        ...
        while self.step < max_steps:
            for batch in tqdm(self.train_dataset,
                ...
                if self.is_master:
                    if self.step % (len(self.train_dataset) * all_n_epochs) == 0:  # save ckpt per 5 epoch
                        self.save_to_checkpoint()
                    # if self.step % (
                    #         len(self.train_dataset) // 2) == 0:  # print loss of a specific batch 3 times an epoch
                    #     print("loss on step no. {:d} is {:2.5f}".format(self.step, loss))
                    #     log_file.flush()

                    if self.step % (len(self.train_dataset) * all_n_epochs) == 0 and self.step !=0:  # vlidation after each epoch
    ...
    def train_step(self, batch):
        ...
        with self.autocast:
            ...
            for _ in range(num_steps):
                ...
                noisy_audio = (noisy_audio - (1.0 - noise_scale) ** 0.5 * predicted) / noise_scale_sqrt
                noisy_audio = torch.clamp(noisy_audio, -1.0, 1.0)  # Clamping to ensure values are within range
                ...
                total_loss_sisdr += -si_sdr(preds=noisy_audio, target=audio_bsm_with_array_rot).mean().item()
    ...
    def validation(self, fast_sampling=False):
        ...
        with torch.no_grad():
            for j, batch in enumerate(self.valid_dataset):
                ...
                audio = torch.randn(audio_bsm_with_array_rot.shape[0], audio_bsm_with_array_rot.shape[1], audio_bsm_with_array_rot.shape[2], device=device)
                ...
                val_err_sisdr_tot += -si_sdr(preds=audio, target=audio_bsm_with_array_rot).mean().item()
            ...
            val_err_sisdr = val_err_sisdr_tot / (j + 1)


            print('Steps : {:d}, Val SI-SDR Error : {:4.3f}'.format(self.step, val_err_sisdr))

            val_err = val_err_sisdr
            if val_err < self.min_val_err:
                self.min_val_err = val_err
...
def train(args, params):
    ...
    valid_dataset = from_path(validation_filelist, len_dataset, params, is_distributed=True)
